...
stoplist = set('for the a of and to in is'.split())
ip_corpus = [[word for word in document.lower().split() if word not in stoplist]
         for document in input_corpus]

#removing words that appear only once
from collections import defaultdict
frequency = defaultdict(int)
for text in ip_corpus:
    for token in text:
         frequency[token] += 1

ip_corpus = [[token for token in text if frequency[token] > 0]
          for text in ip_corpus]
...
batch_size = 128
window_size = 1 #number of words to consider for context (left & right)
num_enum = 2 #number of times to re-use a word to get the label
